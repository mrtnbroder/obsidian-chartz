import {
  ...
  type SankeyControllerDatasetOptions,
  registerables,
} from "chart.js"
...
import { generateInnerColors, renderError } from "./util.js"
import type { ImageOptions } from "./constants/settingsConstants.js"
import type ChartPlugin from "./main.js"
import { generateTableData } from "./chartFromTable.js"
...
export default class Renderer {
  ...
  async datasetPrep(
    ...
    el: HTMLElement,
    themeColors = false,
  ): Promise<{ chartOptions: ChartConfiguration; width: string }> {
    console.log("WATTO")
    let datasets = []
    if (!yaml.id) {
      const colors = []
      if (this.plugin.settings.themeable || themeColors) {
        let i = 1
        while (true) {
          let color = getComputedStyle(el).getPropertyValue(
            `--chart-color-${i}`,
          )
          if (color) {
            colors.push(color)
      ...
      for (let i = 0; yaml.series.length > i; i++) {
        ...
        const dataset = {
          ...
          backgroundColor: yaml.labelColors
            ? colors.length
              ? generateInnerColors(colors, yaml.transparency)
              : generateInnerColors(
                  this.plugin.settings.colors,
                  yaml.transparency,
                )
            : colors.length
              ? generateInnerColors(colors, yaml.transparency)[i]
              : generateInnerColors(
                  this.plugin.settings.colors,
                  yaml.transparency,
                )[i],
          ...
          fill: yaml.fill
            ? yaml.stacked
              ? i == 0
                ...
                : "-1"
              : true
            : false, //See https://github.com/mrtnbroder/obsidian-chartz/issues/53#issuecomment-1084869550
        ...
        if (yaml.type === "sankey") {
          ...
          if (dataset.colorFrom)
            (dataset as SankeyControllerDatasetOptions).colorFrom = (c) =>
              yaml.series[i].colorFrom[c.dataset.data[c.dataIndex].from] ??
              colors[i] ??
              "green"

          if (dataset.colorTo)
            (dataset as SankeyControllerDatasetOptions).colorTo = (c) =>
              yaml.series[i].colorTo[c.dataset.data[c.dataIndex].to] ??
              colors[i] ??
              "green"
        }
    ...
    const gridColor = getComputedStyle(el).getPropertyValue(
    ...
    console.log("Chart.defaults.color", yaml.textColor)
    console.log("Chart.defaults.color", Chart.defaults.color)
    Chart.defaults.font.family =
      getComputedStyle(el).getPropertyValue("--mermaid-font")
    ...
    if (yaml.type == "radar" || yaml.type == "polarArea") {
      ...
      ;(chartOptions as ChartConfiguration<
        "pie" | "doughnut" | "bubble" | "scatter"
      >) = {
        type: yaml.type,
        data: {
          labels,
  ...
  async imageRenderer(yaml: string, options: ImageOptions): Promise<string> {
    ...
    const chartOptions = await this.datasetPrep(
      await parseYaml(
        yaml.replace("```chart", "").replace("```", "").replace(/\t/g, "    "),
      ),
      document.body,
    )
  ...
  renderRaw(data: any, el: HTMLElement): Chart | null {
    console.log("WATTOOOOOAAAAAAAAAAAAOOOOO")
  ...
  async renderFromYaml(
    yaml: any,
    el: HTMLElement,
    ctx: MarkdownPostProcessorContext,
  ) {
    this.plugin.app.workspace.onLayoutReady(() =>
      ctx.addChild(new ChartRenderChild(yaml, el, this, ctx.sourcePath)),
    )
  }
}
    ...
    try {
      ...
      if (this.data.id) {
        ...
        if (this.renderer.plugin.settings.themeable) {
          let i = 1
          while (true) {
            let color = getComputedStyle(this.el).getPropertyValue(
              `--chart-color-${i}`,
            )
            if (color) {
              colors.push(color)
        ...
        if (this.data.file)
          linkDest =
            this.renderer.plugin.app.metadataCache.getFirstLinkpathDest(
              this.data.file,
              this.renderer.plugin.app.workspace.getActiveFile().path,
            )
        const pos = this.renderer.plugin.app.metadataCache
          .getFileCache(
            linkDest ??
              (this.renderer.plugin.app.vault.getAbstractFileByPath(
                this.ownPath,
              ) as TFile),
          )
          .sections.find((pre) => pre.id === this.data.id)?.position
        if (!pos) {
          throw "Invalid id and/or file"
        }

        const tableString = (
          await this.renderer.plugin.app.vault.cachedRead(
            this.data.file
              ? linkDest
              : (this.renderer.plugin.app.vault.getAbstractFileByPath(
                  this.ownPath,
        ...
        ).substring(pos.start.offset, pos.end.offset)
        let tableData
        try {
          tableData = generateTableData(
            tableString,
            this.data.layout ?? "columns",
            this.data.select,
          )
        } catch (error) {
        ...
        for (let i = 0; tableData.dataFields.length > i; i++) {
          x.datasets.push({
            ...
            backgroundColor: this.data.labelColors
              ? colors.length
                ? generateInnerColors(colors, this.data.transparency)
                : generateInnerColors(
                    this.renderer.plugin.settings.colors,
                    this.data.transparency,
                  )
              : colors.length
                ? generateInnerColors(colors, this.data.transparency)[i]
                : generateInnerColors(
                    this.renderer.plugin.settings.colors,
                    this.data.transparency,
                  )[i],
            borderColor: this.data.labelColors
              ...
              : colors.length
                ? colors[i]
                : this.renderer.plugin.settings.colors[i],
            borderWidth: 1,
            fill: this.data.fill
              ? this.data.stacked
                ? i == 0
